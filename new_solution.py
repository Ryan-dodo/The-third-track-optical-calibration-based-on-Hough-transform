...
#     x[i] += 5 * random.random()-random.randint(0,1)
#     y[i] += 5 * random.random()-random.randint(0,1)

# 构建投票矩阵 两条直线的theta取值0.785、2.356，取theta步长0.1
# theta_1     0.7 0.8 0.9
# theta_2     2.2 2.3 2.4
...
vote_list = [[0] * 341 for _ in range(6)]
# rho 取值0-340
# 下面是投票过程
for j in range(0, length_zu):
    for i in theta_1:
        vote_rho = round(x[j] * math.cos(i) + y[j] * math.sin(i))
        vote_list[round((i-0.7)*10)][vote_rho] += 1

for j in range(0, length_zu):
    for i in theta_2:
        vote_rho = round(x[j] * math.cos(i) + y[j] * math.sin(i))
        vote_list[round((i-2.2)*10)+3][vote_rho] += 1
# 展示投票结果
# for _ in vote_list:
#     print(_)
# 找出最多票的点
...
select_line2_y = []
for i in range(len(x)):
    if abs(y[i] - lineB[0] - lineK[0] * x[i]) / math.sqrt(1 + lineK[0] * lineK[0]) < 0.9:
        select_line1_x.append(x[i])
        select_line1_y.append(y[i])
    if abs(y[i] - lineB[1] - lineK[1] * x[i]) / math.sqrt(1 + lineK[1] * lineK[1]) < 0.9:
