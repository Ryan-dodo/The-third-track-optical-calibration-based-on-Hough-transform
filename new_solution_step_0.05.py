...
# theta_1     [0.7,0.75,0.8,0.85,0.9]
# theta_2     [2.2,2.25,2.3,2.35,2.4]

theta_1 = [0.7,0.75,0.8,0.85,0.9]
theta_2 = [2.2,2.25,2.3,2.35,2.4]

vote_list = [[0] * 341 for _ in range(10)]
# rho 取值0-340
# 下面是投票过程
for j in range(0, length_zu):
    for i in theta_1:
        vote_rho = round(x[j] * math.cos(i) + y[j] * math.sin(i))
        vote_list[round((i-0.7)*20)][vote_rho] += 1

for j in range(0, length_zu):
    for i in theta_2:
        vote_rho = round(x[j] * math.cos(i) + y[j] * math.sin(i))
        vote_list[round((i-2.2)*20)+5][vote_rho] += 1
# 展示投票结果
for _ in vote_list:
    print(_)
# 找出最多票的点
